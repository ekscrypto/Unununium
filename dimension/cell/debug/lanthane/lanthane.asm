;; $Header: /cvsroot/uuu/dimension/cell/debug/lanthane/lanthane.asm,v 1.6 2002/08/11 07:41:24 lukas2000 Exp $
...
str_copyright:
     db "Distributed under BSD License",0
;==============================================================================
section .c_init
global _start
_start:
...
;; important note:
;; ---------------
;; The 2 top params _will_ be cleared, that is "ptr to string" and "type" will
;; _not_ be on the stack on return.
;<
						; Saving parameters
  pushad					;------------------------------
  ...
  lea ebp, [esp+0x2c]				; EBP = ptr to 1st arg, if any
						;
						;
						; here we scan for the next 
						; % or 0 so we can print all 
						; those chars at once
						;
.do_span:					;
...
.esc_done:					;
  ...
  jmp .do_span					; and do the next span
						;
.done:						;
  test ecx, ecx					; if there's a span left to
                                                ; print
  jz .zero					;
