;; $Header: /cvsroot/uuu/dimension/cell/fs/ozone/ozone.asm,v 1.3 2002/01/24 14:36:44 jmony Exp $
;; 
;; Ozone VFS cell
...
;==============================================================================
;%define _DEBUG_
%define _TABLE_SIZE_	4		; log 2 number of entries 
					;  in the hash table
...
.enter:						;
  ...
  and edx, (1 << _TABLE_SIZE_) - 1		; convert to a 4 bit
                                                ;  hash value
...
.create_new_hashtable:				;
  ...
  mov edx, HASH_SEED				;
  externfunc lib.string.fasthash		;
  and edx, (1 << _TABLE_SIZE_) - 1		; convert to a 4 bit hash
                                                ;  value
  dbg_print "generated hash ",1			;
  dbg_print_hex edx				;
  dbg_term_log					;
...
.found_it:					;
  ...
  retn						;
						;
.exists:					;
						; the node they are tring to
						; register has already been
						; registered :/
...
.find_node:					;
  ...
  jne .try_next					;
.pass_2:					;
  dbg_print "found the node",0			; found the node, return
  clc						;
  retn						;
						;
.try_next:					;
...
;; 
						;
						; eat up the first char;
						; should be a /. If it's
						; a 0 we have reached the end.
  cmp byte[esi], 0				;
  je .no_more_nodes				;
						;
...
.make_node:					;
  ...
  jc .error					;
						; ebp is unmodified
						;
  mov [ebp], edi				; link the new node up in
                                                ;  the tree
						;
...
;<
  ...
  pop ebp					;
						; all registers except EBX,
						; EDX should be the same as
						; when called
...
%endif						;
						;
  mov eax, fs_descriptor.open			; spill over into _fs_action
;------------------------------------------------------------------------------
...
%endif						;
  ...
  mov [esp+8], edi				;
						;
  mov ebp, esi					; ebp will point to the
                                                ;  sub-string for the fs
  mov ebx, [root_file_descriptor]		; ebx will hold the last found
                                        	;  fs descriptor
						;
  call _get_node				;
  mov edx, HASH_SEED				;
...
.loop:						;
  ...
  jc .partial_match				; if we didn't find the node,
                                                ;  that's bad
						;
  add esi, ecx					;
  call _get_node				;
...
.no_new_fs:					;
  ...
  test eax, eax					; if it's null, we can't find
                                        	;  the file
  jnz .loop					;
						;
.partial_match:					;
...
.found_it:
  ...
  cmp dword[edi+mountpoint_node.fs_descriptor], byte 0		;
  jz .partial_match				; if this node doesn't
                                                ; contain a new FS, it's
						; really
  mov esi, .root_str				; a partial match. If not,
                                                ; we are opening the root
  mov ebx, [edi+mountpoint_node.fs_descriptor]	; of the FS, so set the
  pop edi					; fs descriptor (EBX) and
                                                ; string (ESI) accordingly.
  pop eax					;
  mov edx, ebx					;
  call [ebx+eax]				;
  jc .error					;
...
;<
  ...
  lea ecx, [ecx+edx+5]				; ECX = length of both dirs,
                                                ; a '/' between them,
  externfunc mem.alloc				; and 4 nulls
  jc near .pop1error				;
						;
  mov ecx, ebx					; ECX = length of cwd
  rep movsb					;
  mov byte [edi], '/'				; put a '/' between the CWD
  inc edi					; and file
  pop esi					; restore ptr to supplied dir
  mov ecx, edx					; ECX = length of supplied dir
  rep movsb					;
  mov byte[edi], 0				; dword termination will
  sub edi, edx					; be put on later
  sub edi, ebx					;
  dec edi					; EDI = ptr to full path
                                                ;  (same as after mem.alloc)
...
.absloute:					;
  ...
  lea ecx, [eax-4]				;
  rep movsb					;
  mov byte[edi], 0				; dword termination will be
                                                ; put on later
  lea ecx, [eax-4]				;
  sub edi, ecx					; EDI = ptr to full path
						;
.filter_dots:					;
dbg_print "filtering dots",0			; EDI = ptr to string to filter
						;
  mov [esp], edi				; put our EDI into the EDI
                                                ; to be returned
...
dbg_print "wrapping up",0			;
  ...
  jne .wrap_already				;
  dec edi					; if so, dec edi so we put
                                                ;  0 over it instead
  cmp edi, [esp]				; but first, check if '/' is
                                                ;  the only char
  jne .wrap_already				;
  inc edi					; if so, leave the '/' there
.wrap_already:					;
  mov dword[edi], 0				; terminate the string
  popad						; we put the value to return
                                                ;  in EDI in already
  clc						;
  retn						;
						;
