...
section .c_init
lprint {"[sonar sound system]: SB16 driver version $Revision: 1.3 $.",0xa}, FATALERR
...
mov esi, [blah]
mov ecx ,  0xFFFF
add [blah] , dword 0xFFFF
cmp [soundlength] , dword 0xFFFF
jnb .monk
mov ecx , [soundlength]
retn
.monk:
sub [soundlength] , dword 0xFFFF
retn
...
; 
; if the ecx value changed , it is assumed that
; the last block is going to be played.
;-----------------------------------------------;
...
globalfunc sound.continue_stream_8bit , 721305
mov al , 0x45
call _dsp.write
retn
;
...
globalfunc sound.exit_stream_8bit , 721306
...
cmp dword [callback] , 00h 			;if !0 , then it is pointing to a callbackfunction				;
jne .stream
clc
retn
.stream:

xor [sb.block_no] , byte 1
jz .second_block
call [callback]

mov edx , [sb.block_len]
shr ecx , 1
cmp edx , ecx
je .still_streaming
mov dword  [callback] , _irq_handler.last_block
