...
;------------------------------------------------------------------------------
  ...
  jnc short .rm_transfer_area_allocated		;
						;
    mov edx, 1					; in case it failed...
    mov edi, 0xB8000				; error code: 00000001
  ...
  jnc short .rm_stack_allocated			;
						;
    mov edx, 2					; in case it failed...
    mov edi, 0xB8000				; error code: 00000002
    externfunc debug.diable.dword_out		;
    jmp short $					;
						;
  .rm_stack_allocated:				;
...
.done_fixing:					;
						; Moving cell into it's
						; allocated realmode space
						;------------------------------
  mov esi, cell_start				;
...
;globalfunc realmode.proc_call, 40
  ...
  pushfd					;
  cli						;
						; Save EAX entrance value to be
						; restred prior to INT
  mov [.eax_value - cell_start], eax		;------------------------------
  .fixme000 equ $-4				;
						;
						; Use self-modifying code to
						; fix up interrupt number
						; for the INT
  mov al, [ss:esp + 8]				;
  ...
  mov al, [ss:esp + 11]				;
  out 0xA1, al					;
						; Fix the values to use for
						; for real mode DS and ES
  mov eax, [ss:esp + 12]			;------------------------------
  ...
  .fixme005 equ $-4				;
						; Save the pmode values of DS
						; and ES on stack
						;------------------------------
  push ds					;
  push es					;
						; Save current pmode stack
  mov [.pm_stack_esp - cell_start], esp		;
  .fixme006 equ $-4				;
  mov word [.pm_stack_ss - cell_start], ss	;
  .fixme007 equ $-4				;
						; Save pointer to pmode IDT
						;
...
.es_value equ $-2				;
  mov es, ax					;
						;
						; Set back EAX to what it 
						; was when __interrupt was 
						; first called
  mov eax, 0					;
.eax_value equ $-4				;
						;
  ...
  .interrupt_number equ $-1			;
						;
						; Saving the EAX register, 
						; that will be used to 
						; restore pmode
  mov [cs:.eax_returned-cell_start], eax	;
						;
						; Save the returned flags for
						; further restore
  pushfd					;
  ...
  mov cr0, eax					;
						;
						; Hard encoded far jump 
						; instruction to pmode 
						; re-entry point
						;
						;o32 
						;jmp 0x0008:(.pmode_reenabled
						;               - cell_start)
  db 0x66, 0xEA					;
.fixme009:					;
...
.pm_stack_ss equ $-4				;
  ...
  mov esp, 0					;
.pm_stack_esp equ $-4				;
						; Restore pmode DS and 
						; ES values
  pop es					;
  pop ds					;
						; Restore pmode Interrupt 
						; Descriptor Table
...
.slave_pic_mask equ $-1				;
  out 0xA1, al					;
						;
						; Update the flags to return
						; with the flags returned by
						; the INT routine
						;
  pop eax					;<-- retrieve original EFLAGS
  mov [.ebx_returned - cell_start], ebx		;
  .fixme011 equ $-4				;
  mov ebx, 0					;<-- SMC fixed with EFLAGS
						; returned
.flags_returned equ $-4				;
  and ebx, 0x00000CD7 				; keep only OF,DF,SF,ZF,AF,PF
						; and CF
  and eax, 0xFFFFF32A 				; keep all but OF,DF,SF,ZF,AF
						; ,PF and CF
  or eax, ebx					;
						;
...
[bits 16]
;------------------------------------------------------------------------------
irq0_redirector:				; irq redirection
jmp far [0x8 * 4]				;
						;
irq1_redirector:				;
jmp far [0x9 * 4]				;
...
irq_redirectors:
dw irq0_redirector-cell_start		; jump table
dw irq1_redirector-cell_start		; for irq redirection
