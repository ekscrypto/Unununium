;; $Header: /cvsroot/uuu/uuu/src/cells/dyn_link/executioner/executioner.asm,v 1.19 2002/01/06 18:30:08 daboy Exp $
;;
...
.sym_table_done:			;
  ...
  lea eax, [ecx*4]			; calculate ecx * 28
  shl ecx, 5				;
  sub ecx, eax				; ECX = size of section table
...
.relocate_section:
  ...
  ; do the absloute relocations
  mov ecx, [ebp+ubf_sect.abs_num]
  test ecx, ecx
  jz .abs_done
  ...
  mov esi, [ebx]
  xor edx, edx
  mov eax, [ebp+ubf_sect.abs_reloc]
...
.do_abs_reloc:
  ...
  pop edi
  xchg eax, edi
;  externfunc mem.dealloc	; deallocate reloc table	XXX possible bad dealloc
  xchg eax, edi

.abs_done:
  ; do rel relocs
  
  mov ecx, [ebp+ubf_sect.rel_num]
  test ecx, ecx
  jz .rel_done
  ...
  mov esi, [ebx]
  xor edx, edx
  mov eax, [ebp+ubf_sect.rel_reloc]
