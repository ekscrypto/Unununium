...
;------------------------------------------------------------------------------
init:
  pushad				; backup all regs, we mustn't destroy!
					;
...
init_failed:				;---------------------------------
  mov edi, 0xB8000+(0xA0*25)-0x10	; Memory offset to send error code
  push byte 8				; Error code length
  pop ecx				; set it in ecx for loop control
  mov ah, 0x04				; color code to use
.processing_error_code:			;
  shr edx, 4				; switch digit to display
  mov al, dl				; get digit code
  and al, 0x0F				; mask off useless bits
  cmp al, 0x0A				; differentiate digit/alpha
  sbb al, 0x69				; convert to ascii, part i
  das					; convert to ascii, part ii
  stosw					; send digit to memory
  loop .processing_error_code		; process next digit
  jmp short $				; lock everything up
					;
...
reload_cs:				; reentry point
  ...
  jc short init_failed			; handle error if any
					...
					; Activate IDT
					;-------------
  dec eax				; decrement size of GDT by 1
  push edi				; save gdt location
  o16 push ax				; save size of GDT-1
  lidt [esp]				; set cpu IDTR
  o16 pop ax				; clear off gdt size
...
.registering_idt_entries:		;
  ...
  sti					;
					;
  popad					; restore all regs, we mustn't destroy!
;------------------------------------------------------------------------------
...
strings:
.unhandled_interrupt: db "Unhandled interrupt catched! Locking up.",0
;==============================================================================
...
.no_handler_hooked:				;----------------------------
						...
						;-----------------------------
  inc byte [idt.fatal_error]			; inc fata error count
...
.writing_message:				;
  ...
  jnz short .writing_message			; if char ain't null, continue
