...
set comments=s:;>,m:;;,ex:;<,s:;],m:;\|,ex:;[,:;;

"  Fancy comment auto-formating
set formatoptions+=r
...
" Comments:
syn region  nasmComment		start=";" keepend end="$" contains=nasmInCommentTodo
syn region  nasmSpecialComment	start="^\s*;>\s*$" keepend end="^\s*;<\s*$"
syn keyword nasmInCommentTodo	contained TODO FIXME XXX[XXXXX]
syn cluster nasmGrpComments	contains=nasmInCommentTodo,nasmComment,nasmSpecialComment
...
" stuffs with $ before them are labels
syn match   nasmLabel		"[\$\~]\(\h\|?\)\k*\>"lc=1

" label definitions; must have the ':' after it.
...
syn cluster nasmGrpCntnStruc	contains=ALLBUT,nasmInCommentTodo,nasmMacroDef,@nasmGrpInMacros,@nasmGrpInPreCondits,nasmStructureDef,@nasmGrpInStrucs
...
"  Multi-line macro
syn cluster nasmGrpCntnMacro	contains=ALLBUT,nasmInCommentTodo,nasmStructureDef,@nasmGrpInStrucs,nasmMacroDef,@nasmGrpPreCondits,nasmMemReference,nasmInMacPreCondit,nasmInMacStrucDef
...
syn cluster nasmGrpCntnPreCon	contains=ALLBUT,nasmInCommentTodo,@nasmGrpInMacros,@nasmGrpInStrucs
